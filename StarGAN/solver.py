...
class Solver(object):
    ...
    def __init__(self, celebA_loader, rafd_loader, config):
        ...
        if self.pretrained_model:
            if self.fine_tune == 'Y':
                ...
                Gpre_path = os.path.join(self.pretrained_model_path, '{}_G.pth'.format(self.pretrained_model))
                Dpre_path = os.path.join(self.pretrained_model_path, '{}_D.pth'.format(self.pretrained_model))

                self.load_pretrained_state_dict(self.G, Gpre_path, ['main.0.weight'])
                self.load_pretrained_state_dict(self.D, Dpre_path, ['conv2.weight'])


                for name, param in self.G.named_parameters():
                    if name in ['main.0.weight']:
                        continue
                    else:
    ...
    def build_model(self):
        ...
        if self.dataset == 'Both':
            ...
            self.G = Generator(self.g_conv_dim, self.c_dim, self.g_repeat_num, self.use_pretrained_model)
            #self.Gpre = Generator(self.g_conv_dim, 5, self.g_repeat_num, self.use_pretrained_model)
            self.D = Discriminator(self.image_size, self.d_conv_dim, self.c_dim, self.d_repeat_num, self.use_pretrained_model)
            #self.Dpre = Discriminator(self.image_size, self.d_conv_dim, 5, self.d_repeat_num, self.use_pretrained_model)
    ...
    def load_pretrained_state_dict(self, src_model, filename, except_list=list()):
        ...
        if not except_list:
            pretrained_state_dict = {k: v for k, v in pretrained_state_dict.items() if
                                     k in state_dict and v.size() == state_dict[k].size()}
        else:
        ...
        src_model.load_state_dict(state_dict)
    ...
    def make_celeb_labels(self, real_c):
        """Generate domain labels for CelebA for debuging/testing"""
        ...
        for e in range(start, self.num_epochs):
            for i, (real_x, real_label) in enumerate(self.data_loader):
                ...
                if (i+1) % self.log_step == 0:
                    ...
                    if self.dataset == 'CelebA':
                        print('Classification Acc (closed/front/yawn): ', end='')
                    else:
            ...
            if (e+1) % self.model_save_step == 0:
                ...
                print('Model saved into {}..!'.format(self.model_save_path))
    ...
    def test(self):
        ...
        G_path = os.path.join(self.pretrained_model_path, '{}_G.pth'.format(self.test_model))
